import random,time,datetime,os,csv,json
import numpy as np
...
class Group():
    ...
    def outport_data(self):
        ...
        filename_c=str("group_sm_algorithm_"+"("+
                   str(self.member)+","+
                   str(self.rule)+")_memberud_"+
                   str(self.member_ud)+"_memberdd_"+
                   str(self.member_dd)+"_memberdc_"+
                   str(self.member_dc)+".csv")
        filename_co=str("group_sm_algorithm(origin)_"+"("+
                    str(self.member)+","+
                    str(self.rule)+")_memberud_"+
                    str(self.member_ud)+"_memberdd_"+
                    str(self.member_dd)+"_memberdc_"+
                    str(self.member_dc)+".csv")
        filename_j=str("group_al"+"("+
                   str(self.member)+","+
                   str(self.rule)+")_memberud_"+
                   str(self.member_ud)+"_memberdd_"+
                   str(self.member_dd)+"_memberdc_"+
                   str(self.member_dc)+".json")
        filename_jcdp=str("group_al(d')"+"("+
                      str(self.member)+","+
                      str(self.rule)+")_memberud_"+
                      str(self.member_ud)+"_memberdd_"+
                      str(self.member_dd)+"_memberdc_"+
            ...
            for i in range(0,num):
                if self.dp[i]!='-':
                    if self.dp[i]>0.01:
                        ...
                        c.append(self.c[i])
                        dp.append(self.dp[i])
        with open(filename_co,"w",newline="") as datacsv:
            csvwriter = csv.writer(datacsv,dialect = ("excel"))
            csvwriter.writerow(["c","member false alarm rate","member hit rate",
                                "group false alarm rate","group hit rate","d'","l"])
            for i in range(0,num):
        ...
        with open(filename_j,'a') as f_obj:
            json.dump([self.group_f,self.group_h],f_obj)
        with open(filename_jcdp,'a') as f_obj:
